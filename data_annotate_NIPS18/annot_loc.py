...
def main(args):
    ...
    global CONFIG, DATE, CAM, FRAME_ID, OUT_DIR
    ...
    if args.date:
        ...
        date_folders = sorted(glob(os.path.join(CONFIG['IMG_DIR_BASE'] , '17-*')))
    print('# dates:', len(date_folders))
    # pdb.set_trace()
    for date_folder in date_folders:
        DATE = date_folder.split('\\')[-1] if '\\' in date_folder else date_folder.split('/')[-1]
        if args.cam:
            cam_folders = glob(os.path.join(date_folder, BASE_IP+args.cam))
        else:
            cam_folders = sorted(glob(os.path.join(date_folder, BASE_IP+'*')))
        # print('# cams:', len(cam_folders))
        for cam_folder in cam_folders:
            ...
            depth_frames = sorted(glob(os.path.join(cam_folder, '*.jpg')))
...
def bboxes(depth_frames):
    global CONFIG, DATE, CAM, FRAME_ID, N_FRAMES, PREV_FRAME, OUT_DIR, OCEAN_PATH
...
def centroids(depth_frames, cam, date):
    """
    NOTE: Deprecated.
    """
    global DATE, CAM
    ...
    def save(labels, timestamp):
        labels_fqn = os.path.join(CONFIG['out_dir'], 'centroid_{:s}_{:s}_{:s}.p'.format(DATE, CAM, timestamp))
        with open(labels_fqn, 'wb') as handle:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cam', default='', type=str, help='Last 3 digits of the cam IP. Loop over all cams if not specified.')
