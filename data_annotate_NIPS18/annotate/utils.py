...
#  img = rotate(rotate(img, 1), 1)
  return img

def visualize(depth_image1, white):
  dimage1 = visualize_depth(depth_image1, white)
  return dimage1
...
def get_time_str(t):
  ...
  t = int(os.path.basename(t).split('.')[0])
  t_in_seconds = t / 1000
  ms = t % 1000
...
def draw_info(image, task_state, video_state):
  ...
  image_info = 'Sensor: {}, Frame {}/{}'.format( \
      video_state.depth_sensor, video_state.frame_id+1, video_state.num_frames)
  cv2.putText(image, image_info, (5, 15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)

  time_info = 'Date: {}. Times: depth: {}, thermal: {}'.format(\
      task_state.date, video_state.depth_time, video_state.thermal_time)
  ...
  label_info = ['{}: {}/{}'.format(task_state.actions[action_id],
                ...
                for (start, end, action_id) in [clip for clip in video_state.clips if clip] 
...
def print_info(task_state, video_state):
  ...
  blue_bg('\n              Action              ')
  for action_id, action in task_state.actions.items():
    orange_fg('Action {}: '.format(action_id), action)

  blue_bg('\n           Annotations            ')
  try:
    for clip_id, (start, end, action_id) in enumerate([clip for clip in video_state.clips if clip]):
      orange_fg('Clip {}: '.format(clip_id+1), '[{}, {}], {}'.format(start+1, end+1, task_state.actions[action_id]))
  except:
    print(video_state.clips)
    exit(-1)
  sys.stdout.flush()
...
def read_key(key, task_state, video_state):
  ...
  if key == KEYMAP['<']:
  ...
  elif key == KEYMAP['esc']: # ord('\x1b'):  # esc
    video_state.save()
    return -1
    ...
    if 0 <= tmp < len(video_state.clips):
      ...
      print('Cannot delete {} out of {:d} clips'.format(tmp, len(video_state.clips)))
